...
export interface EmailSetResponse {
  ...
  accountId: string;
  /**
   * The Email state before this set operation.
   */
  oldState: string;
  /**
   * The Email state after this set operation.
   */
  newState: string;
  /**
   * Successfully created emails, keyed by the client-assigned creation id.
   */
  created?: {
    [k: string]: EmailBase & {
      /**
  ...
  /**
   * Map of successfully updated EmailId to null or a partial Email object containing only the properties the server changed (RFC 8620 §5.3).
   */
  updated?: {
    [k: string]:
      | (EmailBase & {
  ...
  /**
   * IDs of successfully destroyed emails, or null if none.
   */
  destroyed?: string[] | null;
  /**
   * Creation ids that failed, with a SetError describing the reason.
   */
  notCreated?: {
    [k: string]: SetError;
  } | null;
  /**
   * EmailIds that failed to update, with a SetError describing the reason.
   */
  notUpdated?: {
    [k: string]: SetError;
  } | null;
  /**
   * EmailIds that failed to destroy, with a SetError describing the reason.
   */
  notDestroyed?: {
    [k: string]: SetError;
  } | null;
