...
/**
 * Arguments for ContactCard/set (RFC 9610 §6.3). Creates, updates, or destroys ContactCard objects.
 */
export interface ContactCardSetArgs {
  ...
  accountId: string;
  /**
   * Optimistic concurrency guard. Fails with stateMismatch if the ContactCard state differs.
   */
  ifInState?: string | null;
  /**
   * Map of client-assigned creation ids to ContactCardCreate objects.
   */
  create?: {
    [k: string]: ContactCardCreate;
  } | null;
  /**
   * Map of ContactCardId to PatchObject. Only explicitly listed properties are changed.
   */
  update?: {
    [k: string]: PatchObject;
  } | null;
  /**
   * ContactCardIds to delete.
   */
  destroy?: readonly string[] | null;
}
/**
 * Fields for creating a new ContactCard. addressBookIds is required.
 */
export interface ContactCardCreate {
  /**
  ...
  version: '1.0';
  /**
   * AddressBookId → true map. Exactly one entry required.
   */
  addressBookIds: {
    [k: string]: boolean;
  };
...
/**
 * Structured name for a ContactCard (RFC 9553 §2.2.1).
 */
