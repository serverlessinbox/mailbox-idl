...
type EmailSubmissionQueryArgs struct {
	...
	Filter map[string]interface{} `json:"filter,omitempty"` // Conditions to filter EmailSubmission objects.
	Sort []map[string]interface{} `json:"sort,omitempty"` // List of Comparator objects specifying the sort order.
