...
type MailboxQueryChangesArgs struct {
	...
	Filter map[string]interface{} `json:"filter,omitempty"` // Must match the filter used in the original Mailbox/query call.
	Sort []map[string]interface{} `json:"sort,omitempty"` // Must match the sort used in the original Mailbox/query call.
