...
type AddressBookQueryArgs struct {
	...
	Filter map[string]any `json:"filter,omitempty"` // Filter conditions. Supported properties: isSubscribed (Boolean).
	Sort []map[string]any `json:"sort,omitempty"` // Sort criteria. Each Comparator has a property (e.g. name) and optional isAscending (default true).
