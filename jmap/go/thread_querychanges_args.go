...
type ThreadQueryChangesArgs struct {
	...
	Filter map[string]any `json:"filter,omitempty"` // Must match the filter used in the original Thread/query call.
	Sort []map[string]any `json:"sort,omitempty"` // Must match the sort used in the original Thread/query call.
