...
type PrincipalQueryArgs struct {
	...
	Filter map[string]any `json:"filter,omitempty"` // Filter conditions. Supported properties: type (String), name (String), email (String), timeZone (String), description (String), hidden (Boolean).
	Sort []map[string]any `json:"sort,omitempty"` // Sort criteria. Each Comparator has a property (e.g. name, type) and optional isAscending (default true).
