...
type MailboxQueryArgs struct {
	...
	Filter map[string]interface{} `json:"filter,omitempty"` // Filter conditions. Supported properties: parentId (MailboxId|null), name (String), role (String|null), hasAnyRole (Boolean), isSubscribed (Boolean).
	Sort []map[string]interface{} `json:"sort,omitempty"` // Sort criteria. Each Comparator has a property (e.g. name, sortOrder) and optional isAscending (default true).
