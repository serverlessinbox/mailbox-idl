...
type EmailSubmissionQueryChangesArgs struct {
	...
	Filter map[string]interface{} `json:"filter,omitempty"` // Must match the filter used in the original query.
	Sort []map[string]interface{} `json:"sort,omitempty"` // Must match the sort used in the original query.
