...
// ContactCardQueryChangesResponse ContactCard/queryChanges response
type ContactCardQueryChangesResponse struct {
	...
	NewQueryState string `json:"newQueryState"` // The current queryState. Pass to the next ContactCard/queryChanges call.
	HasMoreChanges bool `json:"hasMoreChanges"` // If true, more changes exist beyond maxChanges. Call again with newQueryState.
	Removed []string `json:"removed"` // ContactCardIds that are no longer in the query result set.
	Added []map[string]interface{} `json:"added"` // Contact cards that are now in the result set, with their new positions.
