...
type ContactCardQueryArgs struct {
	...
	Filter map[string]interface{} `json:"filter,omitempty"` // Filter conditions. Supported properties: inAddressBook (AddressBookId), uid (String), hasMember (String), kind (String), createdBefore/createdAfter (UTCDate), updatedBefore/updatedAfter (UTCDate), text/name/nickname/organization/email/phone/onlineService/address/note (String).
	Sort []map[string]interface{} `json:"sort,omitempty"` // Sort criteria. Each Comparator has a property (e.g. nickname, created, updated) and optional isAscending (default true).
