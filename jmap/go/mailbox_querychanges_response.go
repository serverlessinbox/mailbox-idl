...
// MailboxQueryChangesResponse Mailbox/queryChanges response
type MailboxQueryChangesResponse struct {
	...
	NewQueryState string `json:"newQueryState"` // The current queryState. Pass to the next Mailbox/queryChanges call.
	HasMoreChanges bool `json:"hasMoreChanges"` // If true, more changes exist beyond maxChanges. Call again with newQueryState.
	Removed []string `json:"removed"` // MailboxIds that are no longer in the query result set.
	Added []map[string]interface{} `json:"added"` // Mailboxes that are now in the result set, with their new positions.
