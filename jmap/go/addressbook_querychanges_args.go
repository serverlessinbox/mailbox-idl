...
type AddressBookQueryChangesArgs struct {
	...
	Filter map[string]any `json:"filter,omitempty"` // Must match the filter used in the original AddressBook/query call.
	Sort []map[string]any `json:"sort,omitempty"` // Must match the sort used in the original AddressBook/query call.
